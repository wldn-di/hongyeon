...
import com.ssafy.s14p11a707.security.oidc.CognitoOidcUserService;
...
/**
 * 세션 기반 Cognito OAuth2 로그인 보안 설정
 * <p>
 * Spring Security의 {@code oauth2Login}을 사용해 AWS Cognito(Hosted UI)로 인증을 위임하고,
 ...
 *   <li>인가 요청은 {@link #authorizationRequestResolver(ClientRegistrationRepository)}에서 PKCE와 {@code identity_provider=Google}을 자동 적용</li>
 * </ul>
 *
 * @see CognitoOidcUserService
...
public class SecurityConfig {
     ...
     * OAuth2 로그인(Cognito Hosted UI)을 연결한다.
     * </p>
     ...
     *   PKCE 적용 + {@code identity_provider=Google} 파라미터 추가로 “즉시 Google로 보내기” 동작을 구현한다.</li>
     *   <li>{@code userInfoEndpoint(...).oidcUserService(...)}:
     *   OIDC 사용자 로딩 시 {@link CognitoOidcUserService}를 사용해 {@code email} 기반 사용자 upsert를 수행한다.</li>
     *   <li>{@code redirectionEndpoint(...).baseUri("/oauth2/code/*")}:
     *   Cognito 콜백 URL을 {@code /oauth2/code/cognito}로 사용하기 위해 리다이렉션 엔드포인트 패턴을 맞춘다.</li>
    ...
    public SecurityFilterChain securityFilterChain(
        ...
        http.oauth2Login(oauth2 -> oauth2
                ...
                .userInfoEndpoint(userInfo -> userInfo
                        .oidcUserService(cognitoOidcUserService)
                )
     ...
     * Cognito가 Google IdP로 즉시 이동하도록 {@code identity_provider=Google} 파라미터를 추가한다.
     * </p>
     *
    ...
    public OAuth2AuthorizationRequestResolver authorizationRequestResolver(
        ...
        resolver.setAuthorizationRequestCustomizer(customizer -> {
            ...
            customizer.additionalParameters(params -> params.put("identity_provider", "Google"));
        });
        return resolver;
    }
