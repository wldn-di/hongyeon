...
@Tag(name = "User(Me) API", description = "내 정보/책장/세션 API")
public interface UserMeApiDoc {

    @Operation(summary = "내 책장 통계 조회", description = "내 책장 통계를 조회합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
    ...
    ResponseEntity<GlobalRankingResponse> getMyRankings();

    @Operation(summary = "내 시나리오 조회", description = "내 시나리오 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
    ...
    @Operation(summary = "미제 사건 조회", description = "내 책장(미제 사건) 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
    ...
    ResponseEntity<ScenarioListResponse> getMyBookshelfFailed();

    @Operation(summary = "완료한 기록 조회", description = "내 책장(완료한 기록) 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
    ...
    ResponseEntity<ScenarioListResponse> getMyBookshelfCompleted();

    @Operation(summary = "진행중 세션 목록 조회", description = "진행중인 세션 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
                    content = @Content(schema = @Schema(implementation = ActiveSessionListResponse.class))
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "잘못된 입력 값",
    ...
    ResponseEntity<ActiveSessionListResponse> getMyActiveSessions();
}
